        ...
        <div style={{width: dimensions.current.width, height: dimensions.current.height, backgroundColor: placement > 7 ? frameColor : "transparent"}}>
            ...
            {placement >= 1 && placement <= 7 &&
                <div className="relative w-full h-full shadow-[8px_8px_8px_#0008] overflow-y-hidden" style={{backgroundColor: portraitBg}}>
                    {char !== null && <Image priority src={`/assets/renders/${char.name === "pikachu" ? (placement === 1 ? "pikachu-lg" : "pikachu-sm") : formatName(char.name)}.webp`} width={2000} height={2000} alt="Char render"
                    ...
                    <p className={`absolute w-fit top-0 left-0 font-extrabold leading-none ${placement > 1 ? placement > 4 ? "text-white px-4 text-[84px]" : "px-6 text-[128px]" : "text-[#fd0] px-8 text-[224px]"}
            ...
            {placement > 7 &&
                <div className="w-full h-full flex items-center gap-1 px-1.5 shadow-[8px_8px_8px_#0008]">
                    ...
                    <div className="relative w-[40px] h-[40px]">
                        {char !== null && <Image src={`${charData[char.index].images[0].url}`} alt={"Char stock icon"} width={2000} height={2000} />}
                    </div>
                    <div className="flex items-center gap-1">
